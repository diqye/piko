...
// 官方组件源码原样落盘（@canvas-ui/flame-wrap-react），不做任何修改。
// WebKit 下 HTML-in-canvas 不可用，组件自动走降级路径：内容正常渲染 + uHasContent=0 的轮廓火焰
import {
  useEffect,
  useRef,
  useState,
...
void main () {
  ...
  if (uHasContent < 0.5) {
    ...
    float aOut = clamp(a + halo * 0.6, 0.0, 1.0);
    // piko 适配：透明窗口上极低 alpha 的浮点残影会在白色桌面下显形成
    // 「模糊矩形」，低于阈值直接归零彻底透明
    if (aOut < 0.004) {
      outColor = vec4(0.0);
      return;
